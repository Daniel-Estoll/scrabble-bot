...
class Scrabble:
    def __init__(self, board, ai_1, ai_2, name_1, name_2):
        ...
        self.opponent_rack, self.opponent_tiles, self.opponent_letters, self.opponent_letter_points = self._create_hand(False)
    ...
    def _update(self):
        ...
        for i, t in enumerate(self.opponent_tiles):
            if i >= len(hand):
                ...
                self.opponent_rack.delete(self.opponent_letters[i])
                self.opponent_rack.delete(self.opponent_letter_points[i])
            else:
                self._update_tile(hand, i, False)
        # User's tiles
    ...
    def _update_tile(self, hand, i, is_user=True):
        if is_user:
            letter_text = self.user_letters[i]
            points_text = self.user_letter_points[i]
        else:
            letter_text = self.opponent_letters[i]
            points_text = self.opponent_letter_points[i]
        letter = hand[i]
        ...
        if is_user:
            ...
            self.opponent_rack.itemconfig(points_text, text=points)
            self.opponent_rack.itemconfig(letter_text, text=letter.upper())
